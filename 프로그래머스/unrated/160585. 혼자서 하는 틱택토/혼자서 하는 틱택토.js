const checkWin = (board, shape) => {
    ...
    if (board[0][1]===shape) {
        if (board[1][1]===shape && board[2][1]===shape) return true;
    }
    if (board[0][2]===shape) {
        if (board[1][2]===shape && board[2][2]===shape) return true;
    }
    if (board[1][0]===shape) {
        if (board[1][1]===shape && board[1][2]===shape) return true;
    }
    if (board[2][0]===shape) {
        if (board[2][1]===shape && board[2][2]===shape) return true;
    }
    if (board[0][2]===shape) {
        if (board[1][1]===shape && board[2][0]===shape) return true;
    }
